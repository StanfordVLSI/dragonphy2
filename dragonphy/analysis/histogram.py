...
class Histogram:
    ...
    @classmethod
    def plot_multi_hist(cls, ffe_out, ideal_in, delay_ffe=0, delay_ideal=0, bins=50, n_bits=1, bit_pos=0, plt_show=True, save_dir=None, print_err=False):
        ...
        bit_patterns = cls.generate_bit_patterns(n_bits) 
    ...
    @classmethod     
    def generate_bit_patterns(cls, n=1):
        return cls.generate_bits(n, [''])

    @classmethod
    def generate_bits(cls, n, bit_list):
        result = []
        for b in bit_list:
            result.append(b+'1')
            result.append(b+'0')
    ...
    @classmethod
    def plot_comparison(cls, ffe_out, ideal, delay_ffe=0, delay_ideal=0, length=100, plt_show=True, save_dir=None, scale=1, labels=None):
        fig = plt.figure()
