...
SW  ISW4 (  .OUT(CS_DRN), .CLK(clk_v2t_l), .CLKB(clk_v2t_lb), .IN(VDD));
SW  ISW3 (  .OUT(CS_DRN), .CLK(clk_v2t_lb), .CLKB(clk_v2t_l), .IN(Vdch));
SW  ISW5 (  .OUT(VDD), .CLK(clk_v2t_e), .CLKB(clk_v2t_eb), .IN(Vdch));
SW  ISW1 (  .OUT(Vch), .CLK(clk_v2t), .CLKB(clk_v2tb), .IN(Vin));
SW  ISW2 (  .OUT(Vch), .CLK(clk_v2tb_gated), .CLKB(clk_v2t_gated), .IN(VSS));
