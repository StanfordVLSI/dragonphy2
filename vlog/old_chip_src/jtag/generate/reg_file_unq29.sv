...
//	[ { Default=>0, IEO=>o, Name=>del_inc_0, Width=>32 },
//	  { Default=>0, IEO=>o, Name=>del_inc_1, Width=>32 },
//	  { Default=>0, IEO=>o, Name=>del_inc_2, Width=>32 },
//	  { Default=>0, IEO=>o, Name=>del_inc_3, Width=>32 }
//	]
//
// BaseAddr (_GENESIS2_INHERITANCE_PRIORITY_) = 0x1c00
//
...
//
// REGISTERS
// del_inc_0 [31:0] IEO=o Offset=0 -- 
// del_inc_1 [31:0] IEO=o Offset=4 -- 
// del_inc_2 [31:0] IEO=o Offset=8 -- 
// del_inc_3 [31:0] IEO=o Offset=12 -- 
...
module reg_file_unq29
  (
   ...
   //outputs
    // outputs for register del_inc_0
    output wire logic[31:0]       del_inc_0_q,
                           
    // outputs for register del_inc_1
    output wire logic[31:0]       del_inc_1_q,
                           
    // outputs for register del_inc_2
    output wire logic[31:0]       del_inc_2_q,
                           
    // outputs for register del_inc_3
    output wire logic[31:0]       del_inc_3_q,
   ...
   assign addr_in_range = ((10'h1c0 == cfgIn_addr_del[13:4]) &&
   ...
   // ==============================
   // register #0 --- name:del_inc_0, type:o, Width:32
   // flop input only on cfg writes
   assign regs_en[0] = onehot_en[0]; 
   // input only from cfg bus
   assign regs_d[0][31:0] = cfgIn_data_del[31:0]; 
   flop_unq4  del_inc_0_reg
     (.Clk(Clk), .Reset(Reset), .en(regs_en[0]),
      .din(regs_d[0][31:0]), .dout(regs_q[0][31:0]));

   // assign value to the relevant output
   assign del_inc_0_q[31:0] = regs_q[0][31:0]; 
   
   // register #1 --- name:del_inc_1, type:o, Width:32
   // flop input only on cfg writes
   assign regs_en[1] = onehot_en[1]; 
   // input only from cfg bus
   assign regs_d[1][31:0] = cfgIn_data_del[31:0]; 
   flop_unq4  del_inc_1_reg
     (.Clk(Clk), .Reset(Reset), .en(regs_en[1]),
      .din(regs_d[1][31:0]), .dout(regs_q[1][31:0]));

   // assign value to the relevant output
   assign del_inc_1_q[31:0] = regs_q[1][31:0]; 
   
   // register #2 --- name:del_inc_2, type:o, Width:32
   // flop input only on cfg writes
   assign regs_en[2] = onehot_en[2]; 
   // input only from cfg bus
   assign regs_d[2][31:0] = cfgIn_data_del[31:0]; 
   flop_unq4  del_inc_2_reg
     (.Clk(Clk), .Reset(Reset), .en(regs_en[2]),
      .din(regs_d[2][31:0]), .dout(regs_q[2][31:0]));

   // assign value to the relevant output
   assign del_inc_2_q[31:0] = regs_q[2][31:0]; 
   
   // register #3 --- name:del_inc_3, type:o, Width:32
   // flop input only on cfg writes
   assign regs_en[3] = onehot_en[3]; 
   // input only from cfg bus
   assign regs_d[3][31:0] = cfgIn_data_del[31:0]; 
   flop_unq4  del_inc_3_reg
     (.Clk(Clk), .Reset(Reset), .en(regs_en[3]),
      .din(regs_d[3][31:0]), .dout(regs_q[3][31:0]));

   // assign value to the relevant output
   assign del_inc_3_q[31:0] = regs_q[3][31:0]; 
   
endmodule: reg_file_unq29
