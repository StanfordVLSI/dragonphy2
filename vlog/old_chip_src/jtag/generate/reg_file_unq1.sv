...
//	[ { IEO=>i, Name=>cal_out_pi, Width=>4 },
//	  { IEO=>i, Name=>addr, Width=>10 }
//	]
//
...
//
// REGISTERS
// cal_out_pi [3:0] IEO=i Offset=0 -- 
// addr [9:0] IEO=i Offset=4 -- 
...
module reg_file_unq1
  (
   ...
   cfg_ifc_unq1.cfgOut cfgOut, // outgoing responds
    ...
    // inputs for register cal_out_pi
    input wire logic [3:0]       cal_out_pi_d,

    // inputs for register addr
    input wire logic [9:0]       addr_d,
   ...
   assign addr_in_range = ((11'h200 == cfgIn_addr_del[13:3]) &&
   ...
   // ==============================
   // register #0 --- name:cal_out_pi, type:i, Width:4
   // flop input with no qualifier
   assign regs_en[0] = 1'b1; 
   // give priority to cfg bus writes, otherwise input from module input
   assign regs_d[0][3:0] = (onehot_en[0])?cfgIn_data_del[3:0]:
                                                                  cal_out_pi_d[3:0]; 
   flop_unq6  cal_out_pi_reg
   ...
   // register #1 --- name:addr, type:i, Width:10
   // flop input with no qualifier
   assign regs_en[1] = 1'b1; 
   // give priority to cfg bus writes, otherwise input from module input
   assign regs_d[1][9:0] = (onehot_en[1])?cfgIn_data_del[9:0]:
                                                                  addr_d[9:0]; 
   flop_unq7  addr_reg
     (.Clk(Clk), .Reset(Reset), .en(regs_en[1]),
      .din(regs_d[1][9:0]), .dout(regs_q[1][9:0]));

   // pad the config bus with zeros
   assign regs_q[1][31:10] = '0; 
