...
//	[ { Default=>7, IEO=>o, Name=>ctl_biasgen_0, Width=>4 },
//	  { Default=>7, IEO=>o, Name=>ctl_biasgen_1, Width=>4 },
//	  { Default=>7, IEO=>o, Name=>ctl_biasgen_2, Width=>4 },
//	  { Default=>7, IEO=>o, Name=>ctl_biasgen_3, Width=>4 }
//	]
//
// BaseAddr (_GENESIS2_INHERITANCE_PRIORITY_) = 0x2700
//
...
//
// REGISTERS
// ctl_biasgen_0 [3:0] IEO=o Offset=0 -- 
// ctl_biasgen_1 [3:0] IEO=o Offset=4 -- 
// ctl_biasgen_2 [3:0] IEO=o Offset=8 -- 
// ctl_biasgen_3 [3:0] IEO=o Offset=12 -- 
...
module reg_file_unq40
  (
   ...
   //outputs
    // outputs for register ctl_biasgen_0
    output wire logic[3:0]       ctl_biasgen_0_q,
                           
    // outputs for register ctl_biasgen_1
    output wire logic[3:0]       ctl_biasgen_1_q,
                           
    // outputs for register ctl_biasgen_2
    output wire logic[3:0]       ctl_biasgen_2_q,
                           
    // outputs for register ctl_biasgen_3
    output wire logic[3:0]       ctl_biasgen_3_q,
   ...
   assign addr_in_range = ((10'h270 == cfgIn_addr_del[13:4]) &&
   ...
   // ==============================
   // register #0 --- name:ctl_biasgen_0, type:o, Width:4
   // flop input only on cfg writes
   assign regs_en[0] = onehot_en[0]; 
   // input only from cfg bus
   assign regs_d[0][3:0] = cfgIn_data_del[3:0]; 
   flop_unq29  ctl_biasgen_0_reg
     (.Clk(Clk), .Reset(Reset), .en(regs_en[0]),
      .din(regs_d[0][3:0]), .dout(regs_q[0][3:0]));

   // assign value to the relevant output
   assign ctl_biasgen_0_q[3:0] = regs_q[0][3:0]; 
   // pad the config bus with zeros
   assign regs_q[0][31:4] = '0; 
   
   // register #1 --- name:ctl_biasgen_1, type:o, Width:4
   // flop input only on cfg writes
   assign regs_en[1] = onehot_en[1]; 
   // input only from cfg bus
   assign regs_d[1][3:0] = cfgIn_data_del[3:0]; 
   flop_unq29  ctl_biasgen_1_reg
     (.Clk(Clk), .Reset(Reset), .en(regs_en[1]),
      .din(regs_d[1][3:0]), .dout(regs_q[1][3:0]));

   // assign value to the relevant output
   assign ctl_biasgen_1_q[3:0] = regs_q[1][3:0]; 
   // pad the config bus with zeros
   assign regs_q[1][31:4] = '0; 
   
   // register #2 --- name:ctl_biasgen_2, type:o, Width:4
   // flop input only on cfg writes
   assign regs_en[2] = onehot_en[2]; 
   // input only from cfg bus
   assign regs_d[2][3:0] = cfgIn_data_del[3:0]; 
   flop_unq29  ctl_biasgen_2_reg
     (.Clk(Clk), .Reset(Reset), .en(regs_en[2]),
      .din(regs_d[2][3:0]), .dout(regs_q[2][3:0]));

   // assign value to the relevant output
   assign ctl_biasgen_2_q[3:0] = regs_q[2][3:0]; 
   // pad the config bus with zeros
   assign regs_q[2][31:4] = '0; 
   
   // register #3 --- name:ctl_biasgen_3, type:o, Width:4
   // flop input only on cfg writes
   assign regs_en[3] = onehot_en[3]; 
   // input only from cfg bus
   assign regs_d[3][3:0] = cfgIn_data_del[3:0]; 
   flop_unq29  ctl_biasgen_3_reg
     (.Clk(Clk), .Reset(Reset), .en(regs_en[3]),
      .din(regs_d[3][3:0]), .dout(regs_q[3][3:0]));

   // assign value to the relevant output
   assign ctl_biasgen_3_q[3:0] = regs_q[3][3:0]; 
   // pad the config bus with zeros
   assign regs_q[3][31:4] = '0; 
   
endmodule: reg_file_unq40
