...
set TDC_inv_area [get_property [get_cells idchain/iTDC_delay_unit_dont_touch_0_/U*] area] 
set TDC_inv_width [expr round($TDC_inv_area/$cell_height/$cell_grid)*$cell_grid]

set TDC_ff_area [get_property [get_cells idchain/iTDC_delay_unit_dont_touch_0_/*_reg] area] 
set TDC_ff_width [expr round($TDC_ff_area/$cell_height/$cell_grid)*$cell_grid]


set delay_chain_width [expr (2*$TDC_inv_width+$TDC_ff_width)*($TDC_Nwidth+2)+$welltap_width]
set delay_chain_height [expr $cell_height*($TDC_Nheight+2)]
set routing_width [expr ceil(5/$cell_grid)*$cell_grid]
set routing_height [expr 8*$cell_height]
...
add_ndr -name NDR_3W3S -spacing {M1:M2 0.04 M3 0.12 M4:M7 0.12} -width {M1:M2 0.12 M3 0.12 M4:M7 0.12} 
...
# controls
editPin -pinWidth [expr $pin_width] -pinDepth $pin_depth -fixOverlap 0 -unit MICRON -spreadDirection clockwise -edge 2 -layer 2 -spreadType start -spacing [expr 4*$metal_space] -offsetStart [expr $boundary_height+2*$cell_height] -pin {ctl_v2t_n* ctl_dcdl* sel* alws_on del_out en_pm en_slice init* rstb}
