...
set BUFT_area [get_property [get_cells iinc_delay_dont_touch/IBUFT0] area]
...
placeInstance idel_PI_dont_touch/idel_PI [expr $welltap_width+$FF_width] 0 
placeInstance iinc_delay_dont_touch/IBUFT0 [expr $FP_width-$welltap_width-$BUFT_width] 0
placeInstance iinc_delay_dont_touch/IBUFT1 [expr $FP_width-$welltap_width-$BUFT_width] $cell_height
...
#########################
addStripe -nets {VDD} \
  -layer M7 -direction vertical -width [expr $AVDD_M7_width/2] -spacing [expr $AVDD_M7_width/2] -start_offset $welltap_width -set_to_set_distance $FP_width -start_from left -switch_layer_over_obs false -max_same_layer_jog_length 2 -padcore_ring_top_layer_limit AP -padcore_ring_bottom_layer_limit M1 -block_ring_top_layer_limit M7 -block_ring_bottom_layer_limit M1 -use_wire_group 0 -snap_wire_center_to_grid None -skip_via_on_pin {standardcell} -skip_via_on_wire_shape {  noshape }  -extend_to all_domains -create_pins 1 

addStripe -nets {VSS} \
  -layer M7 -direction vertical -width [expr $AVDD_M7_width/2] -spacing [expr $AVDD_M7_width/2] -start_offset $welltap_width -set_to_set_distance $FP_width -start_from right -switch_layer_over_obs false -max_same_layer_jog_length 2 -padcore_ring_top_layer_limit AP -padcore_ring_bottom_layer_limit M7 -block_ring_top_layer_limit M7 -block_ring_bottom_layer_limit M7 -use_wire_group 0 -snap_wire_center_to_grid None -skip_via_on_pin {standardcell} -skip_via_on_wire_shape {  noshape } -extend_to all_domains -create_pins 1 
