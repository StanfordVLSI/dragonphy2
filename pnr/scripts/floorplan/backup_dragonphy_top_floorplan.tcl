...
# Use automatic floorplan synthesis to pack macros (e.g., SRAMs) together
    ...
    for {set k 0} {$k<4} {incr k} {
        if {[expr $k % 2] == 0} {
            placeInstance \
            idcore_omm_ffe_i_genblk3_$k\__sram_i_memory \
                [expr $origin_sram_ffe_x + $sram_pair_spacing*($k/2)] \
                $origin_sram_ffe_y2

            placeInstance \
            idcore_oneshot_multimemory_i_genblk3_$k\__sram_i_memory \
    ...
    placeInstance \
    idcore_errt_i/errt_i_sram_i_memory \
    ...
    placeInstance \
    idcore_histogram_inst_core_0_hist_sram_inst_memory \
        [expr $origin_sram_ffe_x + 2*$sram_pair_spacing] \
        $origin_sram_ffe_y2

    placeInstance \
    idcore_histogram_inst_core_1_hist_sram_inst_memory \
        [expr $origin_sram_ffe_x  + 2*$sram_pair_spacing] \
