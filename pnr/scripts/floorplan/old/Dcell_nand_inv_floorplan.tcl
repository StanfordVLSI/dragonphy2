...
placeInstance Iff_ctl  [expr $boundary_width] [expr $boundary_height+$cell_height]
placeInstance Inand  $boundary_width [expr ($Ntri_inv/2+2)*$cell_height] 
placeInstance Inor3  $boundary_width [expr ($Ntri_inv/2-1)*$cell_height] 
placeInstance Ixc2  [expr $FP_width-$boundary_width-$tri_inv_width-$xc7_width-$xc5_width-$xc3_width] [expr ($Ntri_inv/2+1)*$cell_height] 
placeInstance Ixc3  [expr $FP_width-$boundary_width-$tri_inv_width-$xc7_width-$xc5_width-$xc3_width] [expr ($Ntri_inv/2)*$cell_height] 
placeInstance Ixc4  [expr $FP_width-$boundary_width-$tri_inv_width-$xc7_width-$xc5_width] [expr ($Ntri_inv/2+1)*$cell_height] 
placeInstance Ixc5  [expr $FP_width-$boundary_width-$tri_inv_width-$xc7_width-$xc5_width] [expr ($Ntri_inv/2)*$cell_height] 
placeInstance Ixc6  [expr $FP_width-$boundary_width-$tri_inv_width-$xc7_width] [expr ($Ntri_inv/2+1)*$cell_height] 
placeInstance Ixc7  [expr $FP_width-$boundary_width-$tri_inv_width-$xc7_width] [expr ($Ntri_inv/2)*$cell_height] 
...
assignIoPins -align -pin {Z IN1 init_pulse rstb_ff samp slow }

deleteRouteBlk -all

assignIoPins -pin {init rst IN2 OUT}

editPin -pinWidth $pin_width -pinDepth $pin_depth -fixOverlap 1 -unit MICRON -spreadDirection counterclockwise -edge 0 -layer 2 -spreadType start -spacing $metal_space -offsetStart [expr 2*$cell_height] -pin init
...
editPin -pinWidth $pin_width -pinDepth $pin_depth -fixOverlap 1 -unit MICRON -spreadDirection counterclockwise -edge 0 -layer 2 -spreadType start -spacing $cell_height -offsetStart [expr 4*$cell_height] -pin {IN2}
