...
generate
    for(gj=0; gj < numChannels; gj=gj+1) begin
		...
		for(gi=0; gi < seqLength; gi=gi+1) begin
            ...
            assign est_seq[gi][gj][0] = partial_est_seq[gi][gj] - channel_est[gj][gi];
            assign est_seq[gi][gj][1] = partial_est_seq[gi][gj] + channel_est[gj][gi];

            always @(posedge clk or negedge rstb) begin
                if(!rstb) begin
                    est_seq_out[0][gj][gi] <= 0;
                    est_seq_out[1][gj][gi] <= 0; 
                end else begin
                    est_seq_out[0][gj][gi] <= est_seq[gi][gj][0];
                    est_seq_out[1][gj][gi] <= est_seq[gi][gj][1];
                end               
            end
		end
    end
