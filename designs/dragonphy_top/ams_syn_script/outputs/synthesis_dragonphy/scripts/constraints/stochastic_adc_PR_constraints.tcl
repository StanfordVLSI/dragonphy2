...
set_case_analysis 0 [get_ports sel_PFD_in[0]]
set_case_analysis 0 [get_ports sel_PFD_in[1]]
set_case_analysis 0 [get_ports retimer_mux_ctrl_1]
set_case_analysis 0 [get_ports retimer_mux_ctrl_2]
...
#create_clock -name clk_async [get_ports clk_async]  -period [expr $CLK_DIV_PERIOD*0.8] 
#set_clock_uncertainty 0.01  [get_clocks clk_async]

create_clock -name clk_TDC [get_pins idcdl_coarse/out]  -period [expr $CLK_DIV_PERIOD*0.8]
set_clock_uncertainty 0.01  [get_clocks clk_TDC]

#create_clock -name clk_pm -period [expr $CLK_DIV_PERIOD*0.8] [get_pins iPM/iPM_sub/iref_buff1/out]  

create_clock -name clk_TDC_PR -period [expr $CLK_DIV_PERIOD*0.8] [get_pins clk_TDC_phase_reverse_reg/Q]  

create_clock -name clk_retimer -period [expr $CLK_DIV_PERIOD*0.8] [get_ports clk_retimer]  
set_clock_uncertainty 0.01 [get_clocks clk_retimer] 
...
set_clock_groups -asynchronous -group clk_adc -group clk_retimer
...
set_clock_groups -asynchronous -group clk -group clk_retimer
...
#set_clock_groups -asynchronous -group clk_pm -group clk_retimer
...
set_clock_groups -asynchronous -group clk_TDC_PR -group clk_retimer




#create_clock -name clk_v2t_e -period [expr $CLK_DIV_PERIOD*0.8]  -waveform {0 0.25} [get_pins iV2T_clock_gen/clk_div_sampled_reg_dont_touch/Q]  
#create_clock -name clk_v2t -period [expr $CLK_DIV_PERIOD*0.8] -waveform {0 0.25} [get_pins iV2T_clock_gen/idcdl_fine1_dont_touch/out]  
#create_clock -name clk_v2t_l -period [expr $CLK_DIV_PERIOD*0.8] -waveform {0 0.25} [get_pins iV2T_clock_gen/idcdl_fine2_dont_touch/out]  
...
set_false_path -to [get_pins idchain/iTDC_delay_unit_*/phase_reverse_reg/CP] 


#set_disable_timing [get_cells -hierarchical *xc_inv*]
#set_disable_timing [get_cells -hierarchical *ff_out_reg*]
#set_disable_timing [get_cells -hierarchical *phase_reverse_reg*]

set_disable_timing [get_cells  idchain/iTDC_delay_unit*/ff_out_reg/Q_reg]
set_disable_timing [get_cells  idchain/iTDC_delay_unit*/phase_reverse_reg/Q_reg]
set_disable_timing [get_cells  iV2T_clock_gen/iV2T_buffer/iS2D*/iV2T_fixed_xc*/U1]
set_disable_timing [get_cells  iPFD/iarbiter/inand*/U1]

set_output_delay -clock clk_adc -max 0.05 [get_ports {adder_out* sign_out}]
