...
set nor3_area [get_property [get_cells Inor3] area]
set nor3_width [expr round($nor3_area/$cell_height/$cell_grid)*$cell_grid]
...
# Floor Plan #
##############
set FP_width [expr 2*$boundary_width+$ff_width+$tri_inv_width]
...
for { set k 0} {$k < $Ntri_inv} {incr k} {
	...
	placeInstance Itri_inv_$k\_ $x_cor $y_cor
}

placeInstance Iff_ctl  [expr $FP_width-$boundary_width-$ff_width] [expr $boundary_height+$cell_height]
placeInstance Inand  [expr $FP_width-$boundary_width-$ff_width] [expr ($Ntri_inv/2+2)*$cell_height] 
placeInstance Inor3  [expr $FP_width-$boundary_width-$nor3_width] [expr ($Ntri_inv/2-1)*$cell_height] MY

placeInstance Ixc2  [expr $boundary_width+$tri_inv_width+$xc7_width+$xc5_width] [expr ($Ntri_inv/2+1)*$cell_height] MY
placeInstance Ixc3  [expr $boundary_width+$tri_inv_width+$xc7_width+$xc5_width] [expr ($Ntri_inv/2)*$cell_height] R180 
placeInstance Ixc4  [expr $boundary_width+$tri_inv_width+$xc7_width] [expr ($Ntri_inv/2+1)*$cell_height] MY
placeInstance Ixc5  [expr $boundary_width+$tri_inv_width+$xc7_width] [expr ($Ntri_inv/2)*$cell_height] R180
placeInstance Ixc6  [expr $boundary_width+$tri_inv_width] [expr ($Ntri_inv/2+1)*$cell_height] MY
placeInstance Ixc7  [expr $boundary_width+$tri_inv_width] [expr ($Ntri_inv/2)*$cell_height] R180
...
assignIoPins -align -pin {Z init_pulse slow }

deleteRouteBlk -all

assignIoPins -pin {init rst IN1 IN2 OUT}

editPin -pinWidth $pin_width -pinDepth $pin_depth -fixOverlap 1 -unit MICRON -spreadDirection clockwise -edge 2 -layer 2 -spreadType start -spacing $metal_space -offsetStart [expr 2*$cell_height] -pin init
...
editPin -pinWidth $pin_width -pinDepth $pin_depth -fixOverlap 1 -unit MICRON -spreadDirection counterclockwise -edge 3 -layer 3 -spreadType start -spacing $cell_height -offsetStart [expr $FP_width/2] -pin {IN2}
