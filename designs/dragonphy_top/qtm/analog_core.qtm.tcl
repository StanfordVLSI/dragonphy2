...
for {set dim1 0} {$dim1 < 4} {incr dim1} {
    for {set dim2 0} {$dim2 < 9} {incr dim2} {
        create_qtm_port -type input "ctl_pi[$dim1][$dim2]"
        set_qtm_port_load -value 0.010000 "ctl_pi[$dim1][$dim2]"
    }
}
...
for {set dim1 0} {$dim1 < 16} {incr dim1} {
    for {set dim2 0} {$dim2 < 8} {incr dim2} {
        create_qtm_port -type output "adder_out[$dim1][$dim2]"
        set_qtm_port_drive -value 1.000000 "adder_out[$dim1][$dim2]"
    }
}
...
for {set dim1 0} {$dim1 < 2} {incr dim1} {
    for {set dim2 0} {$dim2 < 8} {incr dim2} {
        create_qtm_port -type output "adder_out_rep[$dim1][$dim2]"
        set_qtm_port_drive -value 1.000000 "adder_out_rep[$dim1][$dim2]"
    }
}
...
set_qtm_port_load -value 0.010000 { adbg_intf_i.rstb }

create_qtm_port -type input { adbg_intf_i.en_v2t }
set_qtm_port_load -value 0.010000 { adbg_intf_i.en_v2t }

create_qtm_port -type input { adbg_intf_i.en_slice[15:0] }
set_qtm_port_load -value 0.010000 { adbg_intf_i.en_slice[15:0] }

for {set dim1 0} {$dim1 < 16} {incr dim1} {
    # ctl_v2tn / ctl_v2tp
    for {set dim2 0} {$dim2 < 5} {incr dim2} {
        create_qtm_port -type input "adbg_intf_i.ctl_v2tn[$dim1][$dim2]"
        set_qtm_port_load -value 0.010000 "adbg_intf_i.ctl_v2tn[$dim1][$dim2]"
        create_qtm_port -type input "adbg_intf_i.ctl_v2tp[$dim1][$dim2]"
        set_qtm_port_load -value 0.010000 "adbg_intf_i.ctl_v2tp[$dim1][$dim2]"
    }
    # ctl_v2tn / ctl_v2tp
    for {set dim2 0} {$dim2 < 2} {incr dim2} {
        create_qtm_port -type input "adbg_intf_i.init[$dim1][$dim2]"
        set_qtm_port_load -value 0.010000 "adbg_intf_i.init[$dim1][$dim2]"
    }
}
...
set_qtm_port_load -value 0.010000 { adbg_intf_i.ALWS_ON[15:0] }
...
## TODO: check with Sung-Jin
# create_qtm_constraint_arc -setup -edge rise -from clk_adc -to ctl_valid -value 0.05
# create_qtm_constraint_arc -hold -edge rise -from clk_adc -to ctl_valid -value 0.05

## setup/hold for ctl_pi[35:0]
## TODO: check with Sung-Jin
# for {set idx 0} {$idx < 36} {incr idx} {
#     create_qtm_constraint_arc -setup -edge rise -from clk_adc -to "ctl_pi[$idx]" \\
#         -value 0.05
#     create_qtm_constraint_arc -hold -edge rise -from clk_adc -to "ctl_pi[$idx]" \\
#         -value 0.05
# }
